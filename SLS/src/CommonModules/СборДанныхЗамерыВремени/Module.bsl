...
Процедура Регламент_СобратьДанныеОЗамерахВремениSQL() Экспорт
	...
	|	НастройкиИмпортаЗамеровВремени.КомпьютерВЗамерах КАК КомпьютерВЗамерах
	|ИЗ
...
Процедура Регламент_СобратьДанныеОЗамерахВремениSQLПоНатройке(ВыборкаПараметров)
	...
	ЗагрузитьЗамерыВремениЧерезSQL(ВыборкаПараметров.База1С, ВыборкаПараметров.ПараметрыSQL, ВыборкаПараметров.КомпьютерВЗамерах);
...
Процедура ЗагрузитьЗамерыВремениЧерезSQL(База1С, ПараметрыSQL, КомпьютерВЗамерах)
	...
	ТЧЗамеры.Колонки.Добавить("КомпьютерИмя", ОбщегоНазначения.ОписаниеТипаСтрока(150));
	
	ИменаПолей = ИменаПолей + ",КлючеваяОперацияИмя";
	Если КомпьютерВЗамерах Тогда
		ИменаПолей = ИменаПолей + ",КомпьютерИмя";
	КонецЕсли;
	...
	ЗамерыВремениВБазеSQL(БазаДанныхSQL, ПараметрыSQL, ВремяНачала, ВремяМаксКон, ТЧЗамеры, ИменаПолей, КомпьютерВЗамерах);
	...
	Для Каждого СтрокаЗамера Из ТЧЗамеры Цикл
		...
		Если КомпьютерВЗамерах
			И ЗначениеЗаполнено(СтрокаЗамера.КомпьютерИмя)
			И СтрокаЗамера.КомпьютерИмя <> null
			Тогда
			
			СтрокаНовогоЗамера.Компьютер = СтруктураСерверовПовтИсп.КомпьютерПользователяПоИмени(СокрЛП(СтрокаЗамера.КомпьютерИмя));
		КонецЕсли;
...
Функция ЗамерыВремениВБазеSQL(БазаДанныхSQL, ПараметрыSQL, ВремяНачала, ВремяОкончания, ТЧ_SQL, ИменаПолей, КомпьютерВЗамерах) Экспорт
	...
	|	%ЗамерыВремениРеквизитКомпьютер% AS [КомпьютерИмя]
	...
	Если НЕ КомпьютерВЗамерах Тогда
		ТекстЗапроса_SQL = СтрЗаменить(ТекстЗапроса_SQL, "%ЗамерыВремениРеквизитКомпьютер%", "''");
		ТекстЗапроса_SQL = СтрЗаменить(ТекстЗапроса_SQL, "%ПараметрыСоединенияКомпьютеры%", "");
	Иначе
		
		Если НЕ ЗначениеЗаполнено(ПараметрыSQL.СправочникКомпьютеры) Тогда
			ТекстЗапроса_SQL = СтрЗаменить(ТекстЗапроса_SQL, "%ЗамерыВремениРеквизитКомпьютер%", ПараметрыSQL.ЗамерыВремениРеквизитКомпьютер);
			ТекстЗапроса_SQL = СтрЗаменить(ТекстЗапроса_SQL, "%ПараметрыСоединенияКомпьютеры%", "");
		Иначе
			ТекстЗапроса_SQL = СтрЗаменить(ТекстЗапроса_SQL, "%ЗамерыВремениРеквизитКомпьютер%", "TK.[_Description]");
			ТекстЗапроса_SQL = СтрЗаменить(ТекстЗапроса_SQL, "%ПараметрыСоединенияКомпьютеры%",
				"LEFT JOIN [" + БазаДанныхSQL.Наименование + "].[dbo].[" + ПараметрыSQL.СправочникКомпьютеры + "] as TK
				|	ON src.[" + ПараметрыSQL.ЗамерыВремениРеквизитКомпьютер + "] = TK.[_IDRRef]");
		КонецЕсли;

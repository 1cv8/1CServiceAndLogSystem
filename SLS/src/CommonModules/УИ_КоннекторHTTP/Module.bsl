...
//                             Параметры преобразования см. ДополнительныеПараметры.ПараметрыПреобразованияJSON 
//
Функция PostJson(URL, Json, ДополнительныеПараметры = Неопределено, ПараметрыЗапроса = Неопределено, Сессия = Неопределено) Экспорт
	
	ТекущаяСессия = ?(Сессия = Неопределено, СоздатьСессию(), Сессия);
	ЗаполнитьДополнительныеДанные(ДополнительныеПараметры, ПараметрыЗапроса, Неопределено, Json);
	ПараметрыПреобразованияJSON = 
		ВыбратьЗначение(Неопределено, ДополнительныеПараметры, "ПараметрыПреобразованияJSON", Неопределено);
...
//                             Параметры преобразования см. ДополнительныеПараметры.ПараметрыПреобразованияJSON 
//
Функция PutJson(URL, Json, ДополнительныеПараметры = Неопределено, ПараметрыЗапроса = Неопределено, Сессия = Неопределено) Экспорт
...
Функция КакТекст(Ответ, Кодировка = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(Кодировка) Тогда  
		Если ЗначениеЗаполнено(Ответ.Кодировка) Тогда
...
Функция JsonВОбъект(Json, Кодировка = "utf-8", ПараметрыПреобразования = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Json) тогда
		...
		Исключение
			...
			Если ТипЗнч(Json) = Тип("ДвоичныеДанные") Тогда
				ЧтениеHTML.ОткрытьПоток(Json.ОткрытьПотокДляЧтения(), Кодировка);
			ИначеЕсли ТипЗнч(Json) = Тип("Строка") Тогда
				ЧтениеHTML.УстановитьСтроку(Json);
			Иначе
				ЧтениеHTML.ОткрытьПоток(Json, Кодировка);
			КонецЕсли;
			ПостроительDOM = Новый ПостроительDOM;
			ДокументHTML = ПостроительDOM.Прочитать(ЧтениеHTML);
			Объект = Новый Соответствие;
			Объект.Вставить("error", ?(ДокументHTML.ТекстовоеСодержимое = Неопределено, ДокументHTML.Тело.ТекстовоеСодержимое, ДокументHTML.ТекстовоеСодержимое));
			ЧтениеHTML.Закрыть();
		КонецПопытки;
		...
		Иначе
			Для Каждого Значение Из Значения Цикл
				Если ТипЗнч(Значение) = Тип("Число") тогда
					ЗначениеПараметра = КодироватьСтроку(Формат(Значение, "ЧГ=; ЧН="), СпособКодированияСтроки.КодировкаURL);
				Иначе
...
Процедура ПодготовитьТелоЗапроса(ПодготовленныйЗапрос, Данные, Файлы, Json, ПараметрыЗаписиJSON)
	...
	ИначеЕсли ЗначениеЗаполнено(Данные) Тогда
		...
		Иначе
			...
			КонецЕсли;
			ПодготовленныйЗапрос.Вставить("СтрокаЗапроса", Тело);
	...
	ИначеЕсли Json <> Неопределено Тогда
		...
		СтрокаЗапроса = ОбъектВJson(Json, ПодготовленныйЗапрос.ПараметрыПреобразованияJSON, ПараметрыЗаписиJSON);
		ПодготовленныйЗапрос.Вставить("СтрокаЗапроса", СтрокаЗапроса);
	...
	Иначе
		...
		ПодготовленныйЗапрос.Вставить("СтрокаЗапроса", HTTPЗапрос.АдресРесурса);
	КонецЕсли;
...
Процедура ЗаполнитьДополнительныеДанные(ДополнительныеПараметры, ПараметрыЗапроса, Данные, Json) Экспорт
	...
	Иначе
		Если Не ДополнительныеПараметры.Свойство("Json") = Неопределено Тогда ДополнительныеПараметры.Удалить("Json") КонецЕсли;
		Если Не ДополнительныеПараметры.Свойство("Данные") = Неопределено Тогда ДополнительныеПараметры.Удалить("Данные") КонецЕсли;
		Если Не ДополнительныеПараметры.Свойство("ПараметрыЗапроса") = Неопределено Тогда ДополнительныеПараметры.Удалить("ПараметрыЗапроса") КонецЕсли;
	КонецЕсли;

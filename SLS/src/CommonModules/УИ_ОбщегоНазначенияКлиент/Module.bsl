...
Процедура ЗапуститьКонсольОтладкиПоСсылкеНаДанныеОтладки(СсылкаНаДанныеОтладки) Экспорт
	Если Не ЗначениеЗаполнено(СсылкаНаДанныеОтладки) Тогда
		Возврат;
	КонецЕсли;

	ДанныеОтладки = УИ_ОбщегоНазначенияВызовСервера.СтруктураДанныхОбъектаОтладкиИзСправочникаДанныхОтладки(
		СсылкаНаДанныеОтладки);
...
Процедура ОткрытьФормуРедактированияТекста(Текст, ОписаниеОповещенияОЗакрытии, Заголовок = "") Экспорт
	ПараметрыФормы = Новый Структура;
	...
	Если Подключено Тогда
		...
		Если ЗагружаемоеИмяФайла = "" Тогда
			...
			НачатьПомещениеФайлов(
				...
				ДополнительныеПараметры), , ВыборФайла);
